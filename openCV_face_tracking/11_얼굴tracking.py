...
while True:
    ...
    eyes = eye_cascade.detectMultiScale(
        gray, scaleFactor=1.5, minNeighbors=3, minSize=(10, 10))

    # print(faces)  # 검출된 얼굴 영역이 리스트 형식으로 저장됨 -> .detectMultiScale()함수를 통해 검출한 객체는 리스트 형식이다.

    # 찾은 얼굴이 있으면
    # 얼굴 영역을 영상에 사각형으로 표시
    if len(faces):              # 검출된 얼굴 영역이 있다면 len(faces)는 참이 되고 faces에 리스트 형식으로 저장된 얼굴 영역의 x,y좌표, 너비, 높이를 활용해 사각형 그려주기
